		...
		<div class="card shadow">
			<div class="card-header">
				...
				<div style="display: inline-block; float: right;">
					<button class="btn btn-primary" style="border: 1px; width: 100px;"
				...
				<table class="table text-nowrap" id="tableArea">
					...
					<tr style="height: 400px;">
						<td style="width: 100%;">
								...
								<div id="seatArea"
								style="border: 2px solid black; width: 80%; margin-left: 150px; margin-top: 50px; display: inline-block;">
									<div id="a_line"></div>
									<br />
									<div id="b_line"></div>
									<div id="c_line"></div>
...
// 		</c:forEach>
		...
		for(var i=0; i<seatNumArray.length; i++){
			if(seatNumArray[i].indexOf("A") != -1){	//문자열 a가 담긴 배열값 갯수만큼 버튼 만들기
				...
				optsA += "<button style='border: 0px solid' value=" + seatIdnNumArray[i] + ">" + seatNumArray[i] + "</button>"
			...
			if(seatNumArray[i].indexOf("B") != -1){	
				...
				optsB += "<button style='border: 0px solid' value=" + seatIdnNumArray[i] + ">" + seatNumArray[i] + "</button>"
					+	"<input type='hidden' value=" + avlYnArray[i] + ">&nbsp;&nbsp;&nbsp;"
...
// 					}
			...
			if(seatNumArray[i].indexOf("C") != -1){	
				...
				optsC += "<button style='border: 0px solid' value=" + seatIdnNumArray[i] + ">" + seatNumArray[i] + "</button>"
					+	"<input type='hidden' value=" + avlYnArray[i] + ">&nbsp;&nbsp;&nbsp;"
			}
			$("#c_line").html(optsC);
			...
			if(seatNumArray[i].indexOf("D") != -1){	
				...
				optsD += "<button style='border: 0px solid' value=" + seatIdnNumArray[i] + ">" + seatNumArray[i] + "</button>"
					+	"<input type='hidden' value=" + avlYnArray[i] + ">&nbsp;&nbsp;&nbsp;"
			}
			$("#d_line").html(optsD);
			...
			if(seatNumArray[i].indexOf("E") != -1){	
				...
				optsE += "<button style='border: 0px solid' value=" + seatIdnNumArray[i] + ">" + seatNumArray[i] + "</button>"
					+	"<input type='hidden' value=" + avlYnArray[i] + ">&nbsp;&nbsp;&nbsp;"
			}
			$("#e_line").html(optsE);
			...
			if(seatNumArray[i].indexOf("F") != -1){	
				...
				optsF += "<button style='border: 0px solid' value=" + seatIdnNumArray[i] + ">" + seatNumArray[i] + "</button>"
					+	"<input type='hidden' value=" + avlYnArray[i] + ">&nbsp;&nbsp;&nbsp;"
			}
			$("#f_line").html(optsF);
			...
			if(seatNumArray[i].indexOf("G") != -1){	
				...
				optsG += "<button style='border: 0px solid' value=" + seatIdnNumArray[i] + ">" + seatNumArray[i] + "</button>"
					+	"<input type='hidden' value=" + avlYnArray[i] + ">&nbsp;&nbsp;&nbsp;"
			}
			$("#g_line").html(optsG);
			...
			if(seatNumArray[i].indexOf("H") != -1){	
				...
				optsH += "<button style='border: 0px solid' value=" + seatIdnNumArray[i] + ">" + seatNumArray[i] + "</button>"
					+	"<input type='hidden' value=" + avlYnArray[i] + ">&nbsp;&nbsp;&nbsp;"
			}
			$("#h_line").html(optsH);
		}
