					...
					<tbody class="alignTB">
						<tr>
							...
							<td colspan="2" id="gradEvalRsn">
								<fmt:formatNumber value="${memInfo.rgstSem}" type="number" var="rgstSem" />
								<c:if test="${rgstSem < 8}">
									<label class='badge badge-warning'>등록 학기 졸업 기준(8학기) 미달</label>
								</c:if>
								<fmt:formatNumber value="${stdAcadInfo.ct1Cred}" type="number" var="ct1Cred" />
								<fmt:formatNumber value="${stdAcadInfo.ct1GrdtnCred}" type="number" var="ct1GrdtnCred" />
								<c:if test="${stdAcadInfo.ct1Cred < stdAcadInfo.ct1GrdtnCred}">
									<label class='badge badge-warning'>주전공 이수 학점 졸업 기준 미달</label>
								</c:if>
								<fmt:formatNumber value="${stdAcadInfo.ct2Cred}" type="number" var="ct2Cred" />
								<fmt:formatNumber value="${stdAcadInfo.ct2GrdtnCred}" type="number" var="ct2GrdtnCred" />
								<c:if test="${stdAcadInfo.ct2Cred < stdAcadInfo.ct2GrdtnCred}">
									<label class='badge badge-warning'>복수전공 이수 학점 졸업 기준 미달</label>
								</c:if>
								<fmt:formatNumber value="${stdAcadInfo.ct3Cred}" type="number" var="ct3Cred" />
								<fmt:formatNumber value="${stdAcadInfo.ct3GrdtnCred}" type="number" var="ct3GrdtnCred" />
								<c:if test="${stdAcadInfo.ct3Cred < stdAcadInfo.ct3GrdtnCred}">
									<label class='badge badge-warning'>부전공 이수 학점 졸업 기준 미달</label>
								</c:if>
								<fmt:formatNumber value="${stdAcadInfo.ct4Cred}" type="number" var="ct4Cred" />
								<fmt:formatNumber value="${stdAcadInfo.ct4GrdtnCred}" type="number" var="ct4GrdtnCred" />
								<c:if test="${stdAcadInfo.ct4Cred < stdAcadInfo.ct4GrdtnCred}">
									<label class='badge badge-warning'>교양 이수 학점 졸업 기준 미달</label>
								</c:if>
								<fmt:formatNumber value="${volTime.volRecogTimeSum}" type="number" var="volRecogTimeSum" />
								<fmt:formatNumber value="${volTime.volTimeCrt}" type="number" var="volTimeCrt" />
								<c:if test="${volRecogTimeSum < volTimeCrt}">
...
<script>
	...
	$(function() {
		...
		// 빈 <tr><td></td></tr>는 지운다. (오류의 원인은 찾지 못함 10.19)
