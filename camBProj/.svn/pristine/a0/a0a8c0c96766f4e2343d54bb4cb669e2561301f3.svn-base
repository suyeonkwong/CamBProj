		...
		<div class="card-body">
			<div class="table-responsive">
				<div id="dataTable_wrapper" class="dataTables_wrapper dt-bootstrap4" style="width: 99%">
					<div class="row">
						<div class="col-sm-12">
						<form method="get" action="/professor/grade/list" name="frmSearch" id="frmSearch">
							<input type="hidden" name="pageNo" value="1">
							<input type="hidden" name="inputStatus" id="inputStatus2" value="${param.inputStatus}">
						...
						<span style="color: black; font-weight: bold; float: left; margin-top: 5%;">[미입력 <span style="color: #C02B55"><fmt:formatNumber value="${gradeCount}" pattern="#,###" /></span>건]</span>
...
<!-- 										</th> -->
										...
										<th class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" aria-label="Age: activate to sort column ascending" style="width: 8%;">
											입력 여부
										</th>
									</tr>
								</thead>
						...
						<div id="paging" class="col-sm-12-text-center" style="margin-left: 4.3%;">
								...
								<!-- previous 시작 -->
						            ...
						            <c:if test="${paginationInfo.firstPageNoOnPageList <= 5 }">
						              ...
						              <a href="/professor/grade/list?pageNo=${paginationInfo.firstPageNoOnPageList - 5 }&selectYear=${param.selectYear}&selectSemester=${param.selectSemester}&searchKeyword=${param.searchKeyword}&inputStatus=${param.inputStatus}" aria-controls="example2" data-dt-idx="0" tabindex="0" class="page-link">이전</a>
						            </li>      
						            <!-- previous 끝 -->
									...
									<c:forEach var="pageNo" begin="${paginationInfo.firstPageNoOnPageList }" end="${paginationInfo.lastPageNoOnPageList }" varStatus="stat">
										  <li style="list-style: none;" class="paginate_button page-item <c:if test="${pageNo == param.pageNo || (empty param.pageNo && pageNo == 1)}">active</c:if>">
							                <a href="/professor/grade/list?pageNo=${pageNo}&selectYear=${param.selectYear}&selectSemester=${param.selectSemester}&searchKeyword=${param.searchKeyword}&inputStatus=${param.inputStatus}" aria-controls="example2" data-dt-idx="${pageNo }" tabindex="0" class="page-link">${pageNo }</a>
							              </li>        
						            </c:forEach> 
						            <!-- page번호 끝 -->
