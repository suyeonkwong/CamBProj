<%@ page language="java" contentType="text/html; charset=UTF-8"%>
<%@taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<!DOCTYPE html>
<html>
<body>
	<div class="card shadow mb-4" style="width: 55%; margin:150px 300px;">
		<div class="card-header py-3">
			<h6 class="m-0 font-weight-bold text-primary">수업 등록</h6>
		</div>
		<div class="card-body">
					<div class="row">
						<div class="col-sm-12">
							<table class="table table-bordered dataTable" id="dataTable" width="100%" cellspacing="0" role="grid" aria-describedby="dataTable_info" style="width: 100%;">
								<tr>
									<th class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" aria-label="Position: activate to sort column ascending" style="width: 126px;">
										강의실
									</th>
									<td><input type="text" placeholder="희망 강의실 번호"></td>
								</tr>
								<tr class="even">
									<th class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" aria-label="Office: activate to sort column ascending" style="width: 66px;">
										강의명
									</th>
									<td>
										<div>
											<input type="text"> &nbsp;&nbsp;&nbsp;
											<a href="#" class="btn btn-secondary btn-icon-split" style="margin-bottom: 5px; height: 30px; padding: 0px;">
					                            <span class="text" style="padding-top: 3px;">강의 검색</span>
					                        </a>
				                        </div>
			                        </td>
								</tr>
								<tr class="odd">
									<th class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" aria-label="Age: activate to sort column ascending" style="width: 31px;">
										강의 시간
									</th>
									<td><input type="text" placeholder=""></td>
								<tr class="even">
									<th class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" aria-label="Start date: activate to sort column ascending" style="width: 72px;">
										수강 인원
									</th>
									<td><input type="text" value="" style="width: 10%;"> </td>
								</tr>
								<tr class="even">
									<th class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" aria-label="Start date: activate to sort column ascending" style="width: 72px;">
										학점
									</th>
									<td><input type="text" value="" style="width: 10%;"> </td>
								</tr>
							</table>
						</div>
						&nbsp;&nbsp;&nbsp;
						<a href="#" class="btn btn-secondary btn-icon-split">
                            <span class="text">강의계획서 작성</span>
                        </a>
					</div>
					<br/><br/>
						<div class="row" style=" float: right;">
							<a href="#" class="btn btn-primary btn-icon-split" > 
								<span class="text">확인</span>
							</a>
							&nbsp;&nbsp;
							<a href="#" class="btn btn-primary btn-icon-split" > 
								<span class="text">취소</span>
							</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						</div>	
							
			</div>
	</div>

</body>
</html>