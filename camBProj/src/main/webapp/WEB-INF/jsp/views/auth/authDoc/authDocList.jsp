...
<div id="body">
	...
	<div class="card shadow mb-4">
		...
		<div class="card-header py-3">
			<h6 class="m-0 font-weight-bold text-primary">결재 기안 내역</h6>
		</div>
		...
		<div class="card-body">
							...
							<div class="row marginTop10">
								<div class="col-sm-3">
									<div class="h5 mb-0 mr-3 font-weight-bold text-gray-800">
										<i class="fas fa-spinner text-gray-300"></i> 접수 <span class="text-info">${authDocInfoCount.cnt01}</span> 건
									</div>
								</div>
								<div class="col-sm-3">
									<div class="h5 mb-0 mr-3 font-weight-bold text-gray-800">
										<i class="fas fa-chevron-circle-up text-gray-300"></i> 상신 <span class="text-info">${authDocInfoCount.cnt02}</span> 건
									</div>
								</div>
								<div class="col-sm-3">
									<div class="h5 mb-0 mr-3 font-weight-bold text-gray-800">
										<i class="fas fa-check-circle text-gray-300"></i> 반려 <span class="text-info">${authDocInfoCount.cnt03}</span> 건
									</div>
								</div>
								<div class="col-sm-3">
									<div class="h5 mb-0 mr-3 font-weight-bold text-gray-800">
										<i class="fas fa-times-circle text-gray-300"></i> 결재 완료 <span class="text-info">${authDocInfoCount.cnt04}</span> 건
									</div>
								</div>
							</div>
						</div>
			...
			<div class="row">
				<div class="col-sm-8" style="padding-top: 40px;">
					<label>총 <span class="text-info">${totalCount}</span> 개의 결재 기안이 있습니다. </label>
				</div>
				<div class="col-sm-4 text-right">
					<form class="" action="/auth/authDocList" method="post" id="frmSearch">
						<div class="row marginTop20">
							<div class="form-group col-sm-4">
								<select class="form-control" name="searchCondition" id="searchCondition">
									...
									<option value="authStatCode" <c:if test="${authDocVo.searchCondition=='authStatCode'}">selected</c:if>>처리상태</option>
								</select>
							</div>
							...
							<div class="form-group col-sm-2">
			                	...
			                	<button type="button" class="btn btn-block btn-primary" id="btnSearch">검색</button>
							</div>
						</div>
					</form>
				</div>
							...
							<tbody>
								<c:if test="${authDocList=='[]'}">
									<tr class="text-center">
										<td colspan="5">결재 기안 내역이 없습니다. </td>
									</tr>
								</c:if>
								...
								<c:forEach items="${authDocList}" var="row">
									...
									<tr onclick="location.href='/auth/authDocDetail?authDocNum=${row.authDocNum}'" class="trClick text-center">
										...
										<td>
											<label class="badge ${badgeColor}" style="font-size: 1em; font-weight: normal;">${row.authStatCode}</label>
										</td>
									</tr>
								</c:forEach>
							</tbody>
					...
					<div id="paging" class="col-sm-12 text-center">
		        		<ul class="pagination">
					    	<li style="list-style: none;" class="paginate_button page-item previous <c:if test="${paginationInfo.firstPageNoOnPageList <= 5 }">disabled</c:if>">
					        	<a href="/auth/authDocList?pageNo=${paginationInfo.firstPageNoOnPageList - 5 }" data-dt-idx="0" class="page-link">이전</a>
					        </li>
							...
							<c:forEach var="pageNo" begin="${paginationInfo.firstPageNoOnPageList }" end="${paginationInfo.lastPageNoOnPageList }" varStatus="stat">
					        	<li style="list-style: none;" class="paginate_button page-item <c:if test="${pageNo == param.pageNo || (pageNo==1 && param.pageNo ==null)}">active</c:if>">
					            	<a href="/auth/authDocList?pageNo=${pageNo}" data-dt-idx="${pageNo}" class="page-link">${pageNo }</a>
					            </li>
					        </c:forEach>
					        ...
					        <li style="list-style: none;" class="paginate_button page-item next <c:if test="${paginationInfo.lastPageNoOnPageList == paginationInfo.totalPageCount}">disabled</c:if>">
					          <a href="/auth/authDocList?pageNo=${paginationInfo.lastPageNoOnPageList + 1 }" data-dt-idx="${paginationInfo.lastPageNoOnPageList + 1 }" class="page-link">다음</a>
					        </li>
						</ul>
		        	</div> <!-- //end paging -->
	        	</div>
