		...
		<form id="frmReservation">
			...
			<div class="card-footer"
				...
				<button type="button" class="btn btn-default"
					...
					id="btnDetail" data-toggle="modal" data-target="#exampleModal">예약취소</button>
			</div>
		</form>
	</div>
</div>
	...
	<div class="card-body table-responsive p-0">
		<table class="table text-nowrap">
			...
			<tbody style="text-align: center;">
				<c:if test="${sRRservVo == '[]'}">
					<tr>
						<td colspan="6" class="text-center">예약 정보가 존재하지 않습니다.</td>
					</tr>
				</c:if>
				<c:forEach var="sRRservVo" items="${sRRservVo}">
					<tr>
						<td>${sRRservVo.rnum}</td>
						<td>${sRRservVo.codeName}</td>
...
1) 경상대 그룹스터디룸 문의는 경상대 부속실로 문의해주세요.(042-567-1234)

2) 그룹스터디룸은 정보화시설에서 예약 가능합니다. 그룹스터디룸을 사용할시 로그인을 해주세요.
...
5) 창업관 공사중에는 중앙도서관 스터디룸을 이용해주세요.
                        </textarea>
				</div>
			</div>
		</div>
...
// 		$("#useDate").datepicker({ minDate: 0});
		...
		$("#userCnt").on("propertychange change keyup paste input", function() { //input값 변경시 바로 감지하는 이벤트
			...
			if (parseInt(maxCap) < parseInt(newValue)) {
